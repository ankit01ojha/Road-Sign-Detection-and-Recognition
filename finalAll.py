...
ap.add_argument("-i", "--testImages", required = True ,help = "path to the image")
ap.add_argument("-f", "--file" , required = True ,help = "path to the file where all the queryImages are stored")
args = vars(ap.parse_args())

files = []
files = os.listdir(args['testImages'])

for file in files:
	for realImage in glob.glob('realImages/' + file + '/*.jpeg'):
		print (realImage)

		image = cv2.imread(realImage)
